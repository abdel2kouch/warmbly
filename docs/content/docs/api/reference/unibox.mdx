...
| `campaign_replies` | query | boolean | `true` returns only inbound threads whose latest sender has at least one successfully completed campaign send from this organization. |
...
Rolls up the scope rail and top metric strip in one call: unread, campaign-reply, today, week, snoozed, awaiting-reply, and pending-scheduled counts, plus per-mailbox, per-tag, and per-conversation-label breakdowns. Auth: **Scope** `READ_UNIBOX` · **Org permission** `access_unibox`.
