...
`GET /unibox` also accepts `address=<value>` (matches sender or recipient, for "every conversation with this person"), `direction=sent|received` (resolved against your own mailbox addresses), and `campaign_replies=true` (inbound threads from contacts this organization has successfully emailed through a campaign).
